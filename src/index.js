...
createTodoModal.addEventListener("todoCreate", (e) => {
  ...
  let item = new toDoItem(
    projectIndex,
    projects[projectIndex].toDoListCount++,
    details.todo_name,
    details.date,
    1,
    details.note,
...
projectContent.addEventListener("toggleChecked", (e) => {
  let itemId = e.detail.item_id;
  let projectId = e.detail.project_id;
  projects[projectId].toDoList[itemId].complete =
    !projects[projectId].toDoList[itemId].complete;
  localStorage.setItem("projects", JSON.stringify(projects));
});
...
projectContent.addEventListener("editItem", (e) => {
  ...
  editModal.addEventListener("todoEdit", function (e) {
    ...
    let todoNode = projectContent.querySelector(
      `[data-id="${itemId}"][data-projectid="${projectId}"]`,
    );
    projectContent.replaceChild(
      toDoItemUI(projects[projectId].toDoList[itemId]),
      todoNode,
    );
    localStorage.setItem("projects", JSON.stringify(projects));
  });
